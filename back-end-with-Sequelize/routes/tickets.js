...
module.exports = (sequelizeModels, pusher) => {
    ...
    try {
      ...
      if (owner_id) {
        await addNotification(
          owner_id,
          `Ticket ${title} was deleted and unassigned from you`,
          updater_name
        );
        pusher.trigger(NOTIF_CHANNEL, NOTIF_NEW_EVENT, {
          notif_to_id: owner_id,
        });
      }
  ...
  router.post("/:ticket_id", async (req, res) => {
    try {
      ...
      const oldTicketDataRaw = await Ticket.findAll({ where: { id: id } });
      ...
      if (owner_id && exisiting_owner_id && owner_id === exisiting_owner_id) {
        console.log(
          "ISSUE: ASSIGNING TO THE SAME OWNER FROM: " +
            exisiting_owner_id +
            " TO " +
            owner_id
        );
      ...
      if (owner_id) {
        ...
        await addNotification(
          owner_id,
          `Ticket ${title} is assigned to you`,
          updater_name
        );
      ...
      if (notifMes) {
        pusher.trigger(NOTIF_CHANNEL, NOTIF_NEW_EVENT, {
          notif_to_id: exisiting_owner_id,
        });
      }
  ...
  router.put("/:ticket_id/updateColumn", async (req, res) => {
    try {
      ...
      const oldTicket = await Ticket.findAll({ where: { id: ticketId } });
      const oldColumnId = JSON.parse(JSON.stringify(oldTicket))[0].column_id;

      const oldColumn = await Columns.findAll({ where: { id: oldColumnId } });
      ...
      const newColumn = await Columns.findAll({ where: { id: newColumnId } });
